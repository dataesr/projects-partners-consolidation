...
def identifie_structure(row,cached_data,nom_structure,ville,pays,code_decision):
    if row[nom_structure] in list(cached_data.keys()):
        pass
    else:
        url='https://affiliation-matcher.staging.dataesr.ovh/match'
        f= f"{row[nom_structure]} {row[ville]} {row[pays]}"
        rnsr=requests.post(url, json= {"type":"rnsr","year":"20"+str(row[code_decision][4:6]),"query":f,"verbose":False})
